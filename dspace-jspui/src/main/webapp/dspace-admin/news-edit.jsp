	...
	<form action="<%= request.getContextPath() %>/dspace-admin/news-edit" method="post">
		<div class="form-group">
			...
			<span class="help-block">
				<c:if test="${fn:contains(position, 'top')}">
				...
				<c:if test="${fn:contains(position, 'contact')}">
					<fmt:message key="jsp.dspace-admin.news-edit.text.contact"/>
				</c:if>
				<c:if test="${fn:contains(position, 'sidebar')}">
