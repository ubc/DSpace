 ...
 * @version $Revision$
 */
public class UBCLicenseStep extends AbstractProcessingStep
{
    /***************************************************************************
    ...
    /** log4j logger */
    private static Logger log = Logger.getLogger(UBCLicenseStep.class);
    ...
    protected int processCC(Context context, HttpServletRequest request,
		...
		// unknown license selected
		if (!UBCLicenseUtil.isValidLicense(licenseclass))
		{
    		return STATUS_LICENSE_REJECTED;
		}

		UBCLicenseUtil.assignLicense(licenseclass, item);
		context.commit();
		removeRequiredAttributes(session);
    	return STATUS_COMPLETE;
    }
