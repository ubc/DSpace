...
public class DescribeStep extends AbstractProcessingStep
{
	...
	// whitelist all tags & attributes that can be generated by TinyMCE in textareas
	public static final Whitelist XSS_WHITELIST = Whitelist.basicWithImages().
            ...
            String element, String qualifier, boolean repeated, String lang)
    {
        ...
        else
        {
			...
			value = Jsoup.clean(value, XSS_WHITELIST); // xss filter for tinymce enabled fields
            if (value != null)
            {
                vals.add(value.trim());
            }
        ...
        for (int i = 0; i < vals.size(); i++)
        {
            ...
            String s = vals.get(i);
			s = Jsoup.clean(s, XSS_WHITELIST); // xss filter for tinymce enabled fields
            if ((s != null) && !s.equals(""))
            {
                if (isAuthorityControlled)
                {
