 ...
 * @author gcarpent
 */
public class BlurbEditServlet extends DSpaceServlet
{
    protected void doDSGet(Context c, HttpServletRequest request,
            HttpServletResponse response) throws ServletException, IOException,
            SQLException, AuthorizeException
    {
		request.setAttribute("blurbTypes", BlurbManager.BLURB_TYPES);
            ...
            SQLException, AuthorizeException
    {
        ...
        String blurbType = request.getParameter("position");
        ...
        if (button.equals("submit_edit"))
        {
			...
			news = blurbManager.getBlurb(blurbType);

            //pass the position back to the JSP
            request.setAttribute("position", blurbType);
        ...
        else if (button.equals("submit_save"))
        {
			...
			blurbManager.saveBlurb(blurbType, news);
            //NewsManager.writeNewsFile(blurbType, news);
